...
def display_landmarks_2d(landmarks, hrois, stats):
    ...
    # Figure 1: input data    
    fig_input = plt.figure(figsize=(4.5, 5))
    fig_input.text(.5,.9, "Input activation maps", ha='center')
    vmin, vmax = stats.min(), stats.max()
    for subject in range(n_subjects - 1):
        plt.subplot(n_subjects / 3, 3, subject + 1)
        plt.imshow(stats[subject], interpolation='nearest',
                   vmin=vmin, vmax=vmax)
        plt.axis('off')
    plt.subplots_adjust(left=.02, right=.98, bottom=.02, wspace=.05,
                        hspace=.05, top=.88)
    plt.savefig('bsa_inputs.png')
    """
    ...
    plt.title('Spatial density of blobs', fontsize=10)
    plt.axis('off')
    plt.colorbar(shrink=.8)
