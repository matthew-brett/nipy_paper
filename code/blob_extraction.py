...
dataset = simul.surrogate_2d_dataset(
    n_subj=1, shape=shape, pos=pos, ampli=ampli, width=10.0, seed=4).\
    squeeze()
...
nroi = hroi.HROI_as_discrete_domain_blobs(
    domain, values, threshold=2.0, smin=5)
...
nroi.roi_features['id'] = np.arange(1, 1 + nroi.k)
...
plt.subplot(1, 3, 1)
plt.imshow(dataset, interpolation='nearest', cmap=plt.cm.jet)
# cb = plt.colorbar(anchor=(0.1,0.5), shrink=.8)
plt.axis('off')
...
plt.subplot(1, 3, 3)
plt.imshow(bmap, interpolation='nearest', cmap=plt.cm.jet, 
...
plt.subplots_adjust(right=.98, left=.02, top=.9, bottom=.02, wspace=.02)
